...
class App(object):
  ...
  def resolve_entry(self,path_req):
    """
    Assuming all request paths are absolute starting from given root.
    Relative url are first managed by the browser.
    """
    ...
    if path_req == "/": # root
      ...
      return
    if path_req == ".":
      log.error("got '.' in request path, don't know what to do with it")
      return
    ## non root
    path_parts = path_req.split("/")
    path_parts.remove("") # remove root empty leftover
    print "path parts",path_parts
    ## 1. Try exact resolve from tree path
    ## 2. Try exact resolve from tags
    ## 3. Try partial resolve from tree => could return multiple entries
